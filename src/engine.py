...
class Upsample_promotion(nn.Module):
    ...
    def __init__(self, input_resolution, dim, norm_layer=nn.LayerNorm, norm_flag=1 ):
        super().__init__()
        self.input_resolution = input_resolution
        self.dim = dim
        self.norm_flag = norm_flag
        self.norm = norm_layer(int(dim / 2))
        self.layer = nn.Sequential(nn.Conv2d(dim, 2*dim, 3, 1, 1),
                                   nn.PixelShuffle(2),
                                   )
...
class Upsample(nn.Module):
    ...
    def __init__(self, input_resolution, dim, norm_layer=nn.LayerNorm):
        ...
        self.input_resolution = input_resolution
        self.dim = dim
        self.layer = nn.Sequential(nn.Conv2d(dim, dim, 3, 1, 1),
                                   nn.PixelShuffle(2),
                                   )
...
# -----------------------------------------------------------------------
#   IMAGE GENERATOR (RECOLORER) BASED ON SWIN TRANSFORMER ARCHITECTURE
# -----------------------------------------------------------------------
class SWIN_Generator(nn.Module):
    r""" Swin Transformer
    Args:
    ...
    def __init__(self, img_size=256, patch_size=4, in_chans=3,
                 num_classes=1000,
                 embed_dim=96, depths=[8, 4, 4], num_heads=[8, 8, 8],
        ...
        for i_layer in range(self.num_layers - 1):
            if i_layer == 0:
                layer = BasicLayer(dim=int(embed_dim * 2 ** (self.num_layers - 1 - i_layer)),
                ...
                layer = BasicLayer(dim=int(embed_dim * 2 ** (self.num_layers - 1 - i_layer) * 2),
                                   input_resolution=(
